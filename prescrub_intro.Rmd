...
For a user to upload samples, select the browse button to search through your local machine and choose a file of sample read counts in `.csv` file, in in a 1-row-per-sample format. In this example, we use the plasma data from the SCRuB tutorial (available in our [github repository](https://github.com/Shenhav-and-Korem-labs/SCRuB/])). We recommend that users ensure their data format matches those that we supply.
