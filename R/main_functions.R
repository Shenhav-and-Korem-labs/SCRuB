...
set_up_SCRUB <- function(samples, 
                            ...
                            use_vi = FALSE, 
                            COVERAGE = 10000, 
                            take_average_contam = FALSE, 
                            feast_all_at_once=FALSE ){

  if(take_average_contam){
    contam <- as.matrix( rescale( apply(control_samples, MARGIN = 2, mean ) ) ) %>% t()
  }else{
...
svi_update_scheme <- function(X, gb, G, p, L, lr = .01){
  P <- (p*G) / ( p*G + ( (1-p) %*% t(gb) ) )
  P[is.na(P)] <- 0
  n_controls <- nrow(L)
  n_samples <- nrow(X)
  for(i in sample(n_samples, n_samples, replace=FALSE)){
    # Sample update
    P <- p[i]*G[i, ] / (p[i]*G[i,] + (1-p[i])*gb )
    P[is.na(P)] <- 0
    G[i,] <- (1-lr)*X[i, ] + lr * rescale( X[i,] * P )
    gb <- (1-lr)*gb + lr * rescale( X[i,]*(1-p[i]) )
    p[i] <- (1-lr)*p[i] + lr * sum( X[i,] * p[i] / sum(X[i, ]))
    
    # control update
    k <- sample(n_controls, 1, 1)
    gb <- (1-lr)*gb + lr * rescale(L[k, ])
  }
  
  return(list(new_gamma=G, 
              new_gb=gb, 
              new_p=p))
}
...
converge_samples <- function(X, gb, G, p, max_iters=50, eps = 1e-6){
  a <- 1
  while(a<max_iters){
    # compute updates
    P <- (p*G) / ( p*G + ( (1-p) %*% t(gb) ) )
    P[is.na(P)] <- 0
    new_gamma <- ( X * P ) %>% get_rescaled_mat()
    new_p <- apply( ( X * P ) / apply(X, MARGIN=1, sum), MARGIN=1, sum )
    
    diff <- max(abs(new_gamma-G))
...
#' @param method Either 'EM' (Expectation Maximization) or 'SVI' (Stochastic Variational Inference). The update scheme for SCRuB to use. The SVI scheme requires less memory to run, although it is recommended to always run the 'EM' method if possible.
...
SCRUB_no_spatial <- function(samples, 
  ...
  if(method=='SVI'){
    conv <- converge_samples(X, gb, G, p)
    G <- conv$new_gamma
    p <- conv$new_p
  }
